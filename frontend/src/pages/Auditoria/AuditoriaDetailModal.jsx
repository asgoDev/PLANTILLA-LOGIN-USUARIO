...
export default function AuditoriaDetailModal({ log, onClose }) {
  ...
  const getUserLabel = (log) => {
    if (log.usuario_id && typeof log.usuario_id === 'object') {
      const { nombre, apellido, email } = log.usuario_id;
  ...
  return (
    /* Overlay */
    <div
      className="sticky top-0 inset-0 z-50 flex justify-center p-md mt-0 translate-y-[-5%]"
      style={{ backgroundColor: 'rgba(0,0,0,0.10)', backdropFilter: 'blur(1px)' }}
      onClick={onClose}
    >
      {/* Panel */}
      <div
        className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/20 animate-modal-enter"
